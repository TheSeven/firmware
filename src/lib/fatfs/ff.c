...
static
int _FF_OPTIMIZE chk_chr (const char* str, int chr) {
...
static
FRESULT _FF_OPTIMIZE chk_lock (	/* Check if the file can be accessed */
...
static
int _FF_OPTIMIZE enq_lock (void)	/* Check if an entry is available for a new object */
{
	UINT i;
...
static
UINT _FF_OPTIMIZE inc_lock (	/* Increment object open counter and returns its index (0:Internal error) */
...
static
FRESULT _FF_OPTIMIZE dec_lock (	/* Decrement object open counter */
...
static
void _FF_OPTIMIZE clear_lock (	/* Clear lock entries of the volume */
	FATFS *fs
)
{
	UINT i;
...
#if !_FS_READONLY
static
FRESULT _FF_OPTIMIZE sync_window (
	FATFS* fs		/* File system object */
)
{
	DWORD wsect;
...
static
FRESULT _FF_OPTIMIZE move_window (
	FATFS* fs,		/* File system object */
...
#if !_FS_READONLY
static
FRESULT _FF_OPTIMIZE sync_fs (	/* FR_OK: successful, FR_DISK_ERR: failed */
	FATFS* fs		/* File system object */
)
{
	FRESULT res;
...
DWORD _FF_OPTIMIZE clust2sect (	/* !=0: Sector number, 0: Failed - invalid cluster# */
	FATFS* fs,		/* File system object */
...
DWORD _FF_OPTIMIZE get_fat (	/* 0xFFFFFFFF:Disk error, 1:Internal error, 2..0x0FFFFFFF:Cluster status */
	FATFS* fs,	/* File system object */
...
#if !_FS_READONLY
static
FRESULT _FF_OPTIMIZE remove_chain (
	FATFS* fs,			/* File system object */
...
#if !_FS_READONLY
static
DWORD _FF_OPTIMIZE create_chain (	/* 0:No free cluster, 1:Internal error, 0xFFFFFFFF:Disk error, >=2:New cluster# */
	FATFS* fs,			/* File system object */
...
#if _USE_FASTSEEK
static
DWORD _FF_OPTIMIZE clmt_clust (	/* <2:Error, >=2:Cluster number */
...
static
FRESULT _FF_OPTIMIZE dir_next (	/* FR_OK:Succeeded, FR_NO_FILE:End of table, FR_DENIED:Could not stretch */
...
#if !_FS_READONLY
static
FRESULT _FF_OPTIMIZE dir_alloc (
	DIR* dp,	/* Pointer to the directory object */
...
static
int _FF_OPTIMIZE cmp_lfn (			/* 1:Matched, 0:Not matched */
...
static
int _FF_OPTIMIZE pick_lfn (			/* 1:Succeeded, 0:Buffer overflow */
...
#if !_FS_READONLY
static
FRESULT _FF_OPTIMIZE dir_register (	/* FR_OK:Successful, FR_DENIED:No free entry or too many SFN collision, FR_DISK_ERR:Disk error */
...
static
FRESULT _FF_OPTIMIZE dir_remove (	/* FR_OK: Successful, FR_DISK_ERR: A disk error */
...
static
void _FF_OPTIMIZE get_fileinfo (		/* No return code */
	DIR* dp,			/* Pointer to the directory object */
...
static
FRESULT _FF_OPTIMIZE create_name (
	DIR* dp,			/* Pointer to the directory object */
...
static
FRESULT _FF_OPTIMIZE follow_path (	/* FR_OK(0): successful, !=0: error code */
...
static
int _FF_OPTIMIZE get_ldnumber (		/* Returns logical drive number (-1:invalid drive) */
...
static
BYTE _FF_OPTIMIZE check_fs (	/* 0:FAT boor sector, 1:Valid boor sector but not FAT, 2:Not a boot sector, 3:Disk error */
	FATFS* fs,	/* File system object */
...
static
FRESULT _FF_OPTIMIZE find_volume (	/* FR_OK(0): successful, !=0: any error occurred */
...
)
{
	...
	*rfs = fs;							/* Return pointer to the file system object */
	...
	if (fmt == 1 || (!fmt && (fs->part))) {	/* Not an FAT boot sector or forced partition number */
		UINT i;
		DWORD br[4];
		...
		i = fs->part;						/* Partition number: 0:auto, 1-4:forced */
		if (i) i--;
		do {								/* Find an FAT volume */
			bsect = br[i];
			fmt = bsect ? check_fs(fs, bsect) : 2;	/* Check the partition */
		} while (!fs->part && fmt && ++i < 4);
	}
...
static
FRESULT _FF_OPTIMIZE validate (	/* FR_OK(0): The object is valid, !=0: Invalid */
...
#if _FS_RPATH >= 1
#if _VOLUMES >= 2
FRESULT _FF_OPTIMIZE f_chdrive (
...
FRESULT _FF_OPTIMIZE f_opendir (
...
FRESULT _FF_OPTIMIZE f_closedir (
...
FRESULT _FF_OPTIMIZE f_readdir (
...
FRESULT _FF_OPTIMIZE f_getfree (
...
FRESULT _FF_OPTIMIZE f_truncate (
	FIL* fp		/* Pointer to the file object */
)
{
	FRESULT res;
...
FRESULT _FF_OPTIMIZE f_getlabel (
	const TCHAR* path,	/* Path name of the logical drive number */
...
FRESULT _FF_OPTIMIZE f_setlabel (
...
FRESULT _FF_OPTIMIZE f_forward (
	FIL* fp, 						/* Pointer to the file object */
...
)
{
	...
	part = fs->part;	/* Partition (0:auto detect, 1-4:get from partition table)*/
